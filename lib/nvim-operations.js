...
export async function findNvimSockets() {
  ...
  // Add common socket directories
  const tmpdir = process.env.TMPDIR || "/tmp";
  searchDirs.push(tmpdir);

  // Add XDG runtime directory if it exists
  const xdgRuntimeDir = process.env.XDG_RUNTIME_DIR;
  if (xdgRuntimeDir) {
    searchDirs.push(xdgRuntimeDir);
  }

  // Also check /run/user/$UID for Linux systems
  ...
  for (const baseDir of searchDirs) {
    try {
      ...
      for (const entry of entries) {
        // Pattern 1: Direct socket files (nvim.{pid}.0)
        if (entry.isSocket() && entry.name.match(/^nvim\.\d+\.0$/)) {
          const socketPath = path.join(baseDir, entry.name);
          try {
        ...
        // Pattern 2: nvim directories with subdirectories
        if (entry.isDirectory() && entry.name.startsWith("nvim")) {
          const nvimDir = path.join(baseDir, entry.name);
      ...
      // Can't read this directory, skip silently
    }
  }
